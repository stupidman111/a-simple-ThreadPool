...
void* threadpool_thread(void* threadpool) {
    ...
    while (TRUE) {
        ...
        /* 线程池开关状态 */
        if (pool->shutdown) {//若为true，则关闭线程池
            pthread_mutex_unlock(&(pool->lock));
            printf("thread 0x%x is exiting \n", (unsigned int) pthread_self());
        ...
        pthread_mutex_lock(&(pool->thread_counter));//忙线程上锁
        pool->busy_thr_num++;
...
void* admin_thread(void* threadpool) {
    ...
    while (!pool->shutdown) {
        printf("admin ---------------- \n");
        sleep(DEFAULT_TIME); /* 隔一段时间管理一次 */
        pthread_mutex_lock(&(pool->lock));
        int queue_size = pool->queue_size; /* 当前任务数 */
        int live_thr_num = pool->live_thr_num; /* 当前存活的线程数 */
        pthread_mutex_unlock(&(pool->lock));

        pthread_mutex_lock(&(pool->thread_counter));
        int busy_thr_num = pool->busy_thr_num; /* 忙线程 */
        pthread_mutex_unlock(&(pool->thread_counter));
...
int is_thread_alive(pthread_t tid)
{
   ...
   if (kill_rc == ESRCH)  //线程不存在-ESRCH：No such process
   {
      return FALSE;
   }
   return TRUE;
